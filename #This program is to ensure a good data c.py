...
def check_inputs ():
    ...
    Label(main_window, text="               ") .grid(column=5,row=4)
    Label(main_window, text="               ") .grid(column=2,row=6)
    Label(main_window, text="               ") .grid(column=5,row=6)

    #Check that Full Name is not blank, set error text if blank   
    if len(entry_name.get()) == 0 :
        Label(main_window,fg="red", text="Sorry - this can't be blank, please enter your name") .grid(column=2,row=4)
        input_check = 1

    #Check that Receipt Number is not blank, set error text if blank     
    if len(entry_receipt.get()) == 0 :
        Label(main_window,fg="red", text="Required") .grid(column=5,row=4)
        input_check = 1
    ...
    if (entry_quantity.get().isdigit()) : 
        if  int(entry_quantity.get()) < 1 or  int(entry_quantity.get()) > 500:
            Label(main_window,fg="red", text="1-500 only") .grid(column=5,row=6)
            input_check = 1
    else :
        Label(main_window,fg="red", text="1-500 only") .grid(column=5,row=6)
        input_check = 1      
...
def setup_buttons():
    ...
    Button(main_window,text="Append Details",command=check_inputs) .grid(column=2,row=7,pady=(10,30), sticky=E)
